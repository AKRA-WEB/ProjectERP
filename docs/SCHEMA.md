...
### HR Employee Operations (v073)
- `employee_documents` — Extended with `status` (pending_review|verified|rejected|expired), `uploaded_by_user_id`, `verified_by_user_id`, `verified_at`, `rejected_reason`, `notes`. Indexes on `(status, expiry_date)` and `(employee_id, status)`.
- `employee_emergency_contacts` — Contact name, relationship, phone, alt_phone, address, is_primary. Partial unique index ensures at most one primary per employee. Triggers: `set_employee_emergency_contacts_updated_at` (DROP IF EXISTS guard added).
- `hr_employee_audit_events` — Append-only HR audit log: `employee_id`, `actor_user_id`, `event_type` (varchar 80), `event_payload_json` (JSONB). Indexed on `(employee_id, created_at DESC)` and `event_type`.
- `leave_balance_adjustments` — Records entitlement and used-day corrections with `adjustment_kind`, `delta_days`, `balance_before`, `balance_after`, `reason`.
- `attendance_adjustment_requests` — Staff-submitted corrections with `request_number` generated by `next_doc_number('HRA','seq_hra')`. Status: submitted|approved|rejected|cancelled. Approval mutates `attendance_records` atomically. Triggers: `set_attendance_adjustment_requests_updated_at` (DROP IF EXISTS guard added).

### HR Employee Ops Constraints (v074)
- `leave_balance_adjustments.balance_after` — CHECK `>= 0`
- `attendance_adjustment_requests.requested_status` — CHECK NULL or IN ('present','absent','late','half_day','holiday')
- `attendance_adjustment_requests.requested_ot_hours` — CHECK NULL or `>= 0`
