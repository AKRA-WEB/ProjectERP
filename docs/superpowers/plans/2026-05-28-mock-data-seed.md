...
**Goal:** Create `lib/db/seed.js` that seeds all ERP modules with in-progress mock data so every button/action on every page is clickable without manual data entry.

**Architecture:** Single idempotent Node.js script using `pg` pool, queries existing user/warehouse IDs dynamically, inserts in dependency order with `ON CONFLICT DO NOTHING`, prints progress per module.

**Tech Stack:** Node.js (CommonJS), `pg` (PostgreSQL client), `dotenv` (already loaded by `npm run migrate:seed`)

---

## Pre-flight: Table reference

| Table | Key columns used |
|-------|-----------------|
| `purchase_requisitions` | warehouse_id, requested_by, status::pr_status |
| `pr_line_items` | pr_id, product_id, qty_requested, unit_cost, line_number |
| `purchase_orders` | vendor_id, warehouse_id, status::po_status, subtotal, vat_amount, total_amount, created_by |
| `po_line_items` | po_id, product_id, qty_ordered, unit_price, line_number |
| `pr_po_links` | pr_id, po_id |
| `goods_receipt_notes` | po_id, warehouse_id, status::grn_status, received_by, received_date |
| `grn_line_items` | grn_id, po_line_item_id, product_id, qty_received, qty_accepted, line_number |
| `warehouse_transfers` | source_warehouse_id, dest_warehouse_id, initiated_by, status::transfer_status |
| `warehouse_transfer_lines` | transfer_id, product_id, qty, line_number |
| `rma_requests` | warehouse_id, vendor_id, initiated_by, status::rma_status |
| `rma_line_items` | rma_id, product_id, qty_returned, condition::rma_condition, line_number |
| `vendor_claims` | vendor_id, warehouse_id, status::claim_status, claim_amount, resolution_type::claim_resolution_type, description, created_by |
| `cycle_counts` | warehouse_id, status::cycle_count_status, initiated_by |
| `cycle_count_lines` | cycle_count_id, product_id, qty_system, line_number |
| `sales_quotations` | customer_id, warehouse_id, status::sq_status, valid_until, subtotal, vat_amount, total_amount, created_by |
| `sq_line_items` | sq_id, product_id, qty, unit_price, discount_amount, line_number |
| `sales_orders` | customer_id, warehouse_id, status::so_status, expected_delivery, payment_terms_days, subtotal, vat_amount, total_amount, confirmed_by, confirmed_at, created_by |
| `so_line_items` | so_id, product_id, qty_ordered, qty_delivered, unit_price, discount_amount, line_number |
| `delivery_orders` | so_id, warehouse_id, status::do_status, shipping_address, created_by |
| `do_line_items` | do_id, so_line_item_id, product_id, qty_to_deliver, unit_price, line_number |
| `pos_sessions` | warehouse_id, opened_by, status::pos_session_status, opening_float |
| `pos_members` | name_th, phone, tier, discount_rate, point_balance |
| `leave_types` | code, name_th, name_en, days_per_year, is_paid |
| `leave_balances` | employee_id, leave_type_id, year, days_entitled |
| `leave_requests` | employee_id, leave_type_id, start_date, end_date, days_requested, status |
| `attendance_records` | employee_id, work_date, clock_in, clock_out, status |
| `fiscal_periods` | name, year, month, start_date, end_date, status |
| `journal_entries` | fiscal_period_id, entry_date, entry_type, description, status, created_by |
| `journal_entry_lines` | journal_entry_id, account_id, debit_amount, credit_amount, line_number |
| `bom_headers` | product_id, uom_id, output_qty, bom_type, version, created_by |
| `bom_lines` | bom_id, line_number, component_id, uom_id, qty_required |
| `vendor_rebate_contracts` | vendor_id, threshold_amount, rebate_rate, period::rebate_period_type, valid_from, valid_to |
| `stock_ledger` | product_id, warehouse_id, entry_type::ledger_entry_type, qty_change, reference_id, created_by |

---

## Task 1: Script scaffold + DB helpers
...
Expected: error "No admin user found" OR "✅ Seed complete." depending on whether stub functions exist. The DB connection itself must not throw. If you see a connection error, check `DATABASE_URL` in `.env`.

---

## Task 2: Product categories + products
...
git commit -m "feat(seed): add categories and products"
```

---

## Task 3: Vendors + vendor_products
...
```bash
git add lib/db/seed.js
git commit -m "feat(seed): add vendors and vendor_products"
```
...
```bash
git add lib/db/seed.js
git commit -m "feat(seed): add customers"
```

---

## Task 5: Pricing + POS members
...
Expected:
```
  ✓ product_prices: 120 row(s)   (15 products × 2 channels × 4 tiers)
  ✓ pos_members: 3 row(s)
```
...
```bash
git add lib/db/seed.js
git commit -m "feat(seed): add product prices and POS members"
```

---

## Task 6: Initial stock (stock_ledger)
...
async function seedStock(users, wh, uoms) {
  const prods = await q(`SELECT id, sku FROM products WHERE sku LIKE ANY(ARRAY['BRD-%','PST-%','ING-%','BEV-%','PKG-%','EQP-%'])`);
    ...
    for (const [whCode, qtyIdx] of warehousePairs) {
      await q(
        `INSERT INTO stock_ledger (product_id, warehouse_id, entry_type, qty_change, reference_id, created_by)
         VALUES ($1, $2, 'grn_receipt', $3, NULL, $4)`,
        [pBySku[sku], wh[whCode], qtys[qtyIdx], users.admin.id]
      );
      count++;
    }
  }
...
Expected: `✓ stock_ledger: 45 row(s)` (15 products × 3 warehouses)

Query to verify balances updated:
```sql
SELECT p.sku, w.code, sb.qty_on_hand, sb.qty_available
FROM stock_balances sb
JOIN products p ON p.id = sb.product_id
JOIN warehouses w ON w.id = sb.warehouse_id
WHERE w.code IN ('W1','W2','W3')
...
```bash
git add lib/db/seed.js
git commit -m "feat(seed): add initial stock via stock_ledger"
```

---

## Task 7: Purchase flow (PR → PO → GRN)
...
async function seedPurchaseFlow(users, wh, uoms) {
  ...
  let grn1Rows = await q(
    `INSERT INTO goods_receipt_notes (po_id, warehouse_id, status, received_by, received_date)
     VALUES ($1,$2,'received'::grn_status,$3,CURRENT_DATE - 4)
     ON CONFLICT DO NOTHING RETURNING id`,
  ...
  await q(
    `INSERT INTO grn_line_items (grn_id, po_line_item_id, product_id, qty_received, qty_accepted, line_number)
     VALUES ($1,$2,$3,250,250,1) ON CONFLICT (grn_id, line_number) DO NOTHING`,
  ...
  let grn2Rows = await q(
    `INSERT INTO goods_receipt_notes (po_id, warehouse_id, status, received_by, received_date, qc_reviewed_by, qc_reviewed_at, qc_notes)
     VALUES ($1,$2,'qc_passed'::grn_status,$3,CURRENT_DATE - 2,$4,NOW(),'สินค้าผ่าน QC ทั้งหมด')
     ON CONFLICT DO NOTHING RETURNING id`,
  ...
  await q(
    `INSERT INTO grn_line_items (grn_id, po_line_item_id, product_id, qty_received, qty_accepted, line_number)
     VALUES ($1,$2,$3,50,50,1) ON CONFLICT (grn_id, line_number) DO NOTHING`,
...
Expected output:
```
  ✓ purchase_requisitions (submitted): 1 row(s)
  ✓ purchase_requisitions (converted): 1 row(s)
  ✓ purchase_orders (sent): 1 row(s)
  ✓ purchase_orders (partially_received): 1 row(s)
  ✓ purchase_orders (partially_received #2): 1 row(s)
  ✓ grn (received — QC pending): 1 row(s)
  ✓ grn (qc_passed — ready to stock): 1 row(s)
```
...
```bash
git add lib/db/seed.js
git commit -m "feat(seed): add purchase flow PR/PO/GRN"
```

---

## Task 8: Sales flow (SQ → SO → DO)
...
async function seedSalesFlow(users, wh, uoms) {
  ...
  let sqRows = await q(
    `INSERT INTO sales_quotations (customer_id, warehouse_id, status, valid_until, subtotal, vat_amount, total_amount, created_by, sent_at)
     VALUES ($1,$2,'sent'::sq_status, CURRENT_DATE + 30, $3,$4,$5,$6, NOW())
     ON CONFLICT DO NOTHING RETURNING id`,
  ...
  let so1Rows = await q(
    `INSERT INTO sales_orders (customer_id, warehouse_id, status, expected_delivery, payment_terms_days, subtotal, vat_amount, total_amount, confirmed_by, confirmed_at, created_by)
     VALUES ($1,$2,'confirmed'::so_status, CURRENT_DATE + 3, 30, $3,$4,$5,$6,NOW(),$6)
     ON CONFLICT DO NOTHING RETURNING id`,
  ...
  const so2Sub = 100 * pBySku['BRD-001'].price + 50 * pBySku['ING-001'].price; // ING-001 included in prods query above
  const so2Vat = Math.round(so2Sub * 0.07 * 100) / 100;
  let so2Rows = await q(
    `INSERT INTO sales_orders (customer_id, warehouse_id, status, expected_delivery, payment_terms_days, subtotal, vat_amount, total_amount, confirmed_by, confirmed_at, created_by)
     VALUES ($1,$2,'partially_delivered'::so_status, CURRENT_DATE + 5, 45, $3,$4,$5,$6,NOW(),$6)
     ON CONFLICT DO NOTHING RETURNING id`,
...
Expected:
```
  ✓ sales_quotations (sent): 1 row(s)
  ✓ sales_orders (confirmed): 1 row(s)
  ✓ delivery_orders (ready): 1 row(s)
  ✓ sales_orders (partially_delivered): 1 row(s)
```
...
```bash
git add lib/db/seed.js
git commit -m "feat(seed): add sales flow SQ/SO/DO"
```
...
Expected: `✓ pos_sessions (open): 1 row(s)`
...
```bash
git add lib/db/seed.js
git commit -m "feat(seed): add open POS session"
```

---

## Task 10: HR (leave types, balances, requests, attendance)
...
async function seedHR(users, wh, uoms) {
  ...
  await q(
    `INSERT INTO leave_requests (employee_id, leave_type_id, start_date, end_date, days_requested, status, notes)
     VALUES ($1,$2,'2026-06-02','2026-06-03',2,'pending','ขอลาพักร้อนประจำปี')
     ON CONFLICT DO NOTHING`,
    [users.staff.id, ltByCode['LT-ANN']]
  );
  log('leave_requests (pending)', 1);
  ...
  for (let dayOffset = 9; dayOffset >= 0; dayOffset--) {
    const dateQ = await q(`SELECT (CURRENT_DATE - $1)::date AS d`,[dayOffset]);
    ...
    for (const user of workUsers) {
      const r = await q(
        `INSERT INTO attendance_records (employee_id, work_date, clock_in, clock_out, status)
         VALUES ($1,$2,($2::timestamp + interval '8 hours'),($2::timestamp + interval '17 hours'),'present')
...
```
  ✓ leave_types: 3 row(s)
  ✓ leave_balances: (N × 3) row(s)
  ✓ leave_requests (pending): 1 row(s)
  ✓ leave_requests (approved): 1 row(s)
  ✓ attendance_records: ~N row(s)
```
...
```bash
git add lib/db/seed.js
git commit -m "feat(seed): add HR leave types, balances, requests, attendance"
```

---

## Task 11: Accounting (fiscal periods + journal entries)
...
async function seedAccounting(users, wh, uoms, accounts) {
  ...
  for (let year = 2025; year <= 2026; year++) {
    for (let month = 1; month <= 12; month++) {
      ...
      const r = await q(
        `INSERT INTO fiscal_periods (name, year, month, start_date, end_date, status)
         VALUES ($1,$2,$3,$4,$5,'open'::fiscal_period_status)
...
Expected:
```
  ✓ fiscal_periods: 24 row(s)
  ✓ journal_entries (draft): 1 row(s)
  ✓ journal_entries (posted): 1 row(s)
```
...
```bash
git add lib/db/seed.js
git commit -m "feat(seed): add accounting fiscal periods and journal entries"
```

---

## Task 12: Misc (Transfer, RMA, Claim, Cycle Count)
...
Expected:
```
  ✓ warehouse_transfers (pending): 1 row(s)
  ✓ rma_requests (open): 1 row(s)
  ✓ vendor_claims (open): 1 row(s)
...
```bash
git add lib/db/seed.js
git commit -m "feat(seed): add transfer, RMA, claim, cycle count"
```

---

## Task 13: BOM + Rebate contract
...
Expected — all modules print row counts, final line: `✅ Seed complete.`
...
```bash
git add lib/db/seed.js
git commit -m "feat(seed): add BOM and rebate contracts"
```
...
| URL | Expected |
|-----|----------|
| `/dashboard` | Stats show (non-zero inventory) |
| `/products` | 15 products listed |
| `/vendors` | 5 vendors listed |
| `/customers` | 8 customers listed |
| `/purchase-requests` | 2 PRs: 1 Approve button, 1 historical |
| `/purchase-orders` | 3 POs: 1 "sent" (receive button), 2 "partially received" |
| `/grn` | 2 GRNs: 1 QC button, 1 Stock button |
| `/transfers` | 1 pending transfer with Complete button |
| `/rma` | 1 open RMA with Resolve button |
| `/claims` | 1 open claim with Resolve button |
| `/cycle-counts` | 1 counting cycle count with Submit/Approve buttons |
| `/sales-orders` | 2 SOs in list |
| `/delivery-orders` or `/sales-orders` | 1 DO with Ship button |
| `/pos` | Open session ready at W1 |
| `/hr` | Leave requests visible, attendance calendar populated |
| `/accounting` | Fiscal periods + 2 JEs |
| `/bom` | 1 BOM for Sandwich Bread |
...
```bash
git add lib/db/seed.js
git commit -m "feat(seed): complete mock data seed for all ERP modules"
```
