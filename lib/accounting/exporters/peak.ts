...
export function exportPeak(rows: ExportRow[]): { filename: string; mime: string; buffer: Buffer } {
  // PEAK Journal Entry template format
  const headers = ['Journal_Date', 'Journal_ID', 'Journal_Name', 'Detail', 'Account_Code', 'Account_Name', 'Debit_Amount', 'Credit_Amount'];
  ...
  for (const r of rows) {
    const line = [
      ...
      escapeCsvValue(r.entry_type.toUpperCase()),
  ...
  return {
    filename: `PEAK_GL_${new Date().toISOString().split('T')[0]}.csv`,
