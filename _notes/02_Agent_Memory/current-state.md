...
- **hardening-t2-ci-gate**: Enforced `qa:verify` through `simple-git-hooks` pre-push and GitHub Actions. Added hard-rule gates for explicit `any`, empty catches, test floor, local-rule suppression baseline, and unbounded-query baseline. `npm run qa:verify` and `npm run agent:closeout` pass (2026-06-13).
...
- **Hardening T7 / Scale Tooling Foundation (planning only)**: Added `conductor/tracks/hardening-t7-scale-tooling-foundation/plan.md` for API contract inventory/OpenAPI, Playwright smoke harness, observability readiness, and domain-boundary audit. Depends on completed `hardening-t1-test-foundation` and `hardening-t2-ci-gate`; no runtime code, dependencies, or DB migration has been implemented yet.
