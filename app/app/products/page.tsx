        ...
        <Table>
          <Thead>
            <tr>
              ...
              <Th className="hidden sm:table-cell">หมวดหมู่ / Category</Th>
              <Th className="hidden sm:table-cell">หน่วย / UOM</Th>
              <Th className="hidden sm:table-cell">ราคาทุน / Cost</Th>
              <Th className="hidden sm:table-cell">ติดตาม / Tracking</Th>
              <Th>สถานะ / Status</Th>
              <Th></Th>
            </tr>
          </Thead>
            ...
            {loading ? (
              ...
              data?.data.map((p) => (
                <tr key={p.id} className="hover:bg-gray-50">
                  ...
                  <Td className="hidden sm:table-cell text-sm text-gray-500">{p.category_name ?? 'โ€”'}</Td>
                  <Td className="hidden sm:table-cell text-sm">{p.uom_code}</Td>
                  <Td className="hidden sm:table-cell text-sm">{formatCurrency(p.unit_cost)}</Td>
                  <Td className="hidden sm:table-cell">
                    {p.is_lot_tracked && <Badge variant="blue">Lot</Badge>}
                    {p.is_serial_tracked && <Badge variant="purple">Serial</Badge>}
                    {!p.is_lot_tracked && !p.is_serial_tracked && <span className="text-gray-400 text-xs">โ€”</span>}
                  </Td>
                  <Td>
