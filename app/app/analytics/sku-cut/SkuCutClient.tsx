...
interface Props {
  ...
  initialPerformance: { rows: PerformanceSku[]; total: number };
}
...
export function SkuCutClient({ initialCandidates, initialPerformance }: Props) {
  ...
  const [performance, setPerformance] = useState<PerformanceSku[]>(initialPerformance.rows);
  const [loading, setLoading] = useState(false); // we have initial data!
  ...
  const [totalPerformance, setTotalPerformance] = useState(initialPerformance.total);
  const LIMIT = 25;
  ...
  const loadData = useCallback(async () => {
    // Skip fetching if at initial state (page 1, no search, no bucket filter) to prevent double fetch
    if (page === 1 && !searchQuery && !bucketFilter) {
      if (activeTab === 'candidates') {
        setCandidates(initialCandidates);
        return;
      } else {
        setPerformance(initialPerformance.rows);
        setTotalPerformance(initialPerformance.total);
        return;
      }
    }
    ...
    setLoading(false);
  }, [activeTab, searchQuery, bucketFilter, page, loadCandidates, loadPerformance, initialCandidates, initialPerformance]);
