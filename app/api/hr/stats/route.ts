...
export async function GET() {
  ...
  // 2. Department count
  const [deptStats] = await query<{ count: string }>(
    `SELECT COUNT(*) FROM departments WHERE is_active = TRUE`,
    []
  );

  // 3. Pending Leave Requests
  const [leaveStats] = await query<{ pending: string }>(
  ...
  const [attendanceStats] = await query<{ present: string; late: string; absent: string }>(
    `SELECT 
  ...
  const latestPayroll = await queryOne<{ run_number: string; period_month: number; period_year: number; status: string }>(
