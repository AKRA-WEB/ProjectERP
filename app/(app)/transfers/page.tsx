  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        ...
        <Link href="/app/transfers/new"><Button>+ สร้างการโอน</Button></Link>
      </div>

      <div className="rounded-xl bg-white shadow-sm border border-gray-100">
        <Table>
          <Thead>
            <tr><Th>เลขที่โอน</Th><Th>จากคลัง</Th><Th>ไปคลัง</Th><Th>ผู้ดำเนินการ</Th><Th>รายการ</Th><Th>สถานะ</Th><Th>วันที่</Th><Th></Th></tr>
          </Thead>
          <Tbody>
            {loading ? (
              ...
              <tr key={t.id} className="hover:bg-gray-50">
                ...
                <Td className="text-sm">{t.source_code} — {t.source_name}</Td>
                <Td className="text-sm">{t.dest_code} — {t.dest_name}</Td>
                <Td className="text-sm">{t.initiated_by_name}</Td>
                <Td className="text-sm text-center">{t.line_count}</Td>
                <Td><StatusBadge status={t.status} /></Td>
                <Td className="text-sm text-gray-500">{formatDate(t.created_at)}</Td>
                <Td><Link href={`/app/transfers/${t.id}`} className="text-sm text-blue-600 hover:underline">ดู</Link></Td>
              </tr>
            ))}
          </Tbody>
        </Table>
