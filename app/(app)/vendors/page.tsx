  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        ...
        <Button onClick={openNew}>+ เพิ่มผู้จำหน่าย</Button>
      </div>
      ...
      <div className="rounded-xl bg-white shadow-sm border border-gray-100">
        <Table>
          <Thead>
            <tr><Th>รหัส</Th><Th>ชื่อ</Th><Th>เลขผู้เสียภาษี</Th><Th>ติดต่อ</Th><Th>เครดิต (วัน)</Th><Th>สถานะ</Th><Th></Th></tr>
          </Thead>
          <Tbody>
            {loading ? (
              ...
              <tr key={v.id} className="hover:bg-gray-50">
                ...
                <Td className="text-sm text-gray-500">{v.tax_id ?? '—'}</Td>
                <Td className="text-sm text-gray-600">
                  {v.contact_name && <div>{v.contact_name}</div>}
                  {v.phone && <div className="text-xs text-gray-400">{v.phone}</div>}
                </Td>
                <Td className="text-sm text-center">{v.payment_terms_days}</Td>
                <Td>
