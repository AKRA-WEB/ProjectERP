  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        ...
        <Link href="/app/claims/new"><Button>+ สร้างการเรียกร้อง</Button></Link>
      </div>
      ...
      <div className="rounded-xl bg-white shadow-sm border border-gray-100">
        <Table>
          <Thead>
            <tr><Th>เลขที่</Th><Th>ผู้จำหน่าย</Th><Th>คลัง</Th><Th>ผู้สร้าง</Th><Th>มูลค่า</Th><Th>วิธีแก้ไข</Th><Th>สถานะ</Th><Th>วันที่</Th><Th></Th></tr>
          </Thead>
          <Tbody>
            {loading ? (
              ...
              <tr key={c.id} className="hover:bg-gray-50">
                ...
                <Td className="text-sm">{c.vendor_code} — {c.vendor_name}</Td>
                <Td className="text-sm">{c.warehouse_code}</Td>
                <Td className="text-sm">{c.created_by_name}</Td>
                <Td className="text-sm font-medium">{formatCurrency(c.claim_amount)}</Td>
                <Td className="text-sm text-gray-500">{c.resolution_type?.replace(/_/g, ' ') ?? '—'}</Td>
