            ...
            {loading ? (
              ...
              data?.data.map((pr) => (
                <tr key={pr.id} className="hover:bg-gray-50">
                  ...
                  <Td className="text-sm">{pr.warehouse_code} — {pr.warehouse_name}</Td>
                  <Td className="text-sm">{pr.requested_by_name}</Td>
                  <Td className="text-sm text-center">{pr.line_count}</Td>
                  <Td><StatusBadge status={pr.status} /></Td>
                  <Td className="text-sm text-gray-500">{formatDate(pr.created_at)}</Td>
                  <Td>
